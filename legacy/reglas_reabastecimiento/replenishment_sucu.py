...
def main():
    ...
    existing_orderpoints = models.execute_kw(db, uid, password,
        ...
        [[]],
        {'fields': ['product_id', 'location_id', 'route_id']}
    )
    existing_keys = {
        (op['product_id'][0], op['location_id'][0], op['route_id'][0] if op['route_id'] else None) for op in existing_orderpoints
    }
    print(f"Orderpoints existentes: {len(existing_keys)}")

    # Crear reglas de abastecimiento para productos válidos en SUCU y ruta automática
    created_product_ids = []
    errored_products = []
    location_id = 64  # ubicación SUCU
    route_id = 29     # ruta reabastecimiento automático
    ...
    for product in products:
        ...
        key = (product_id, location_id, route_id)
        ...
        try:
            ...
            print(f"🆕 Creada regla para producto ID {product_id} en ubicación {location_id} y ruta {route_id} (manual)")
